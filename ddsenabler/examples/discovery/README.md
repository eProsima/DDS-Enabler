...
It launches a DDS Enabler that listens to the DDS network and reports the **name** of every topic, service and action it discovers. For topics and services it additionally exposes the **JSON data placeholder(s)** of the relevant type(s): the JSON skeleton (with default values) you would fill in to publish a sample or send a request/reply.
